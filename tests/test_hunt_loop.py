...
class HuntLoopTests(unittest.TestCase):
    ...
    def test_missing_jirachi_logs_miss_then_reboots(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            sav = root / "port2.sav"
            sav.write_bytes(b"working")
    ...
    def test_one_miss_kills_and_reboots_dolphin(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            sav = root / "port2.sav"
            sav.write_bytes(b"working")
            ...
            self.assertEqual(pal_flags, [False, True])
            attempts = hunt_log.attempts_path.read_text(encoding="utf-8")
            self.assertEqual(attempts.count("sv=-1"), 1)
            self.assertIn("result=shiny", attempts)

    def test_receive_error_is_a_miss_and_reboots_dolphin(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            sav = root / "port2.sav"
            sav.write_bytes(b"working")
    ...
    def test_fail_recovers_in_game_without_killing_dolphin(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            sav = root / "port2.sav"
            sav.write_bytes(b"working")
